...
import 'package:monitor_broiler/peternak/riwayatDataPage.dart';
import 'dart:async';

class MonitoringPagePtk extends StatefulWidget {
  const MonitoringPagePtk({super.key});

  @override
  State<MonitoringPagePtk> createState() => _MonitoringPagePtkState();
}

class _MonitoringPagePtkState extends State<MonitoringPagePtk> {
  double zblower = 0.0;
  double zheater = 0.0;
  double zcoolPad = 0.0;
  double suhu = 0.0;
  ...
  Future<void> _insertSensor() async {
    try {
      var res = await http.post(
        ...
        //Uri.parse("http://10.10.203.29/monitoring_api/insertdata_kandang.php"),
...
class DrawerContentPtk extends StatelessWidget {
  const DrawerContentPtk({super.key});
      ...
      child: ListView(
        ...
        children: [
          ...
          DrawerList(
              ...
              rute: (context) => const MonitoringPagePtk()),
          DrawerList(
              listTitle: "Riwayat Monitoring", rute: (context) => Riwayat()),
          DrawerList(
...
class VarContainer extends StatelessWidget {
  const VarContainer({super.key, required this.variabel, required this.nilai});
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Container(
        width: lebarMedia,
        height: (tinggiMedia - 200) / 4,
        decoration:
            const BoxDecoration(color: Color.fromARGB(255, 42, 103, 37)),
        child: Padding(
          padding: const EdgeInsets.all(20),
          child: Text(
            "$variabel\n$nilai",
...
class DrawerList extends StatelessWidget {
  const DrawerList({super.key, required this.listTitle, required this.rute});
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      onTap: () {
        Navigator.pushReplacement(context, MaterialPageRoute(builder: rute));
      },
    );
  }
}
